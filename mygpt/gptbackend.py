...
def draft_response(symptoms, gpt_model = GPT4):
    """
    Parameters:
...
def format_link_list(disease, gpt_model = GPT4):
    """
    Parameters:
...
def get_text_from_link(link, timeout=3):
    #headers = {
    ...
    try:
        html_output = requests.get(link, headers=headers, timeout=timeout)
        html_output.raise_for_status()  # This will raise an error if the HTTP request returned an unsuccessful status code
    except requests.RequestException:
        return "Defective"

    soup = BeautifulSoup(html_output.content, 'html.parser')
    return soup.get_text()  # This gets the entire textual content of the page
...
def format_scrape(link, gpt_model = "gpt-3.5-turbo-16k"):
    ...
    try:
        scrape = get_text_from_link(link)
        if scrape == "Defective":
            return "Defective"
    except:
        return "Defective"
...
def ask_for_clarifier(symptoms, guesses, gpt_model = GPT35):
    """
    Takes 5 guesses and thinks of a clarifier
    """
    lead_text = """
    You are DocMD, a medical student studying general practice. A patient has approached you with the following symptoms: """ + symptoms + """, and the patient thinks the cause might be """ + guesses + """. Please think of one quick
    clarifying question to ask the patient to help you narrow down the diagnosis. You can ask the patient any yes/no question you want, but you should ask something that will help you narrow down the diagnosis. For example, you could ask
    the patient whether they have a fever, or whether they have a cough. You should not ask the patient for their name, or whether they have a headache, since these questions will not help you narrow down the diagnosis.
    Do not output anything else but the question. Do not output any boilerplate. Do not output any statements. Just output the question. It needs to be a yes/no question. The question needs to ba answerable with yes or no.
...
def get_score(symptoms, disease, source, gpt_model="gpt-3.5-turbo"):
    """
    Parameters:
    symptoms: A list of symptoms input as a string, likely written by the end-user from input() in the draft or output by the front end in the final. (i.e. the first thing passed to draft_response)
    disease: the name of a disease. (i.e. something passed into format_link_list, or output in the list from format_diagnosis_list)
    ...
    lead_text = """
    You are a bot that looks at symptoms and overview pages of diseases. It is your task to output a number that gives an indication of how likely it is that the patient has the disease.
    You are not a doctor, but you are a bot that looks at symptoms and overview pages of diseases. It is your task to output a number that gives an indication of how likely it is that the patient has the disease.
    It is importat that you always output a number. If there is little overlap between symptoms and the disease, output a low number. If there is a lot of overlap, output a high number. If there is no overlap, output 0.
    The maximum number you can output is 100. Do not output a number over 100.
    These are the symptoms: """ + symptoms + """, This is the potential disease """ + disease + """.
    Here is a resource on """ + disease + """:

    """

    end_text = """
    Please only answer with a number between 0 and 100. 
    For example:
    3
    63
    Do not respond with anything other than a number between 0 and 100.
    0 means that it is highly unlikely that the patient has the disease.100 means that it is very likely.
    """
    prompt = lead_text + source + end_text
    ...
    return disease + ',' + str(response["choices"][0]["message"]["content"])
